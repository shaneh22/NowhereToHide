...
const generateAsteroid = (num)=>{
    ...
    if(num === 2){
        let newAsteroid = $(`<div class = "asteroid2"></div>`);
        if(randomHeight > .8){
            newAsteroid.css('top','285px');
        }
        else if(randomHeight > .6){
            newAsteroid.css('top', '140px');
        }
        else {
            newAsteroid.css('top', '15px');
        }
...
let isAlive = setInterval(()=>{
    ...
    let shipTop = parseInt($('#spaceship').css('top'));
    ...
    if(asteroid2 != null){
        ...
        asteroid2collision = asteroid2Left < 125 && asteroid2Left > 10 && shipTop < asteroid2Bottom && shipTop + 50 > asteroid2Top;
    }

    if((asteroidLeft < 125 && asteroidLeft > 10 && shipTop < asteroidBottom && shipTop + 50 > asteroidTop) || (asteroid2collision)){
        alert('You died !');
